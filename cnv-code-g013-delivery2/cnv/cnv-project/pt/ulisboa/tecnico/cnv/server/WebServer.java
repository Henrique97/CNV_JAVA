	...
	static class MyHandler implements HttpHandler {
		...
		public void handle(final HttpExchange t) throws IOException {
			try{
				...
				double estimatedCargo = 0;
				int jobId = 0;
				long multiplierIv = 0;
                long multiplierIs = 0;
				String serverIp="localhost:8000";
				// Store as if it was a direct call to SolverMain.
				final ArrayList<String> newArgs = new ArrayList<>();
				for (final String p : params) {
					final String[] splitParam = p.split("=");
					if(splitParam[0].equals("cargo")){
						estimatedCargo = Double.parseDouble(splitParam[1]);
						continue;
					} else if (splitParam[0].equals("jobid")) {
						jobId = Integer.parseInt(splitParam[1]);
						continue;
					} else if (splitParam[0].equals("multiplierIs")) {
                        multiplierIs = Long.parseLong(splitParam[1]);
                        continue;
                    } else if (splitParam[0].equals("multiplierIv")) {
                        multiplierIv = Long.parseLong(splitParam[1]);
                        continue;
                    }
				...
				String[] ip = t.getRemoteAddress().toString().split(":");
				serverIp = "http://" + ip[0].replaceAll("/","");
				//Communicate to Statistics Analyzer the estimated cargo for this request
				StatisticsAnalyzer.setupJob(estimatedCargo, jobId, serverIp, multiplierIs, multiplierIv);
